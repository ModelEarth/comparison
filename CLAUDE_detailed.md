...
There are **6 tradeflow files** per country (3 flows × 2 file types) plus 2 reference files:
...
**⚠️ CRITICAL WARNING:** Only request data for these 14 countries. Requesting non-existent countries causes 404 errors that can create infinite retry loops and crash the browser.
...
- ✅ Country flags in tooltips
- ✅ Auto-rotate labels when many countries
- ✅ Responsive layout
...
- 🟢 **Exports:** `#10b981` (Green)
- 🟠 **Imports:** `#f59e0b` (Orange)

**Features:**
- ✅ Stacked bars showing flow composition
...
**Features:**
- ✅ Horizontal bars for long industry names
- ✅ Top 15 industries only (prevents clutter)
- ✅ Purple gradient colors
- ✅ Text overflow handling
- ✅ Aggregates across all trade flows
...
1. ✅ **Data Structure** - Complete file organization and schema
2. ✅ **Core Configuration** - Countries, factors, global state
3. ⏳ **Chart Components** (3 of 8 completed):
   - Country Comparison Bar Chart
   - Trade Flow Breakdown
   - Industry Breakdown
4. ⏳ **Remaining sections to add:**
   - Dual Pie Charts
...
- ✅ Country selector outside charts (prevents overlap)
- ✅ Donut style pie charts
- ✅ Percentage labels on slices
- ✅ Consistent color scheme
- ✅ Legend below charts (not covering data)
...
**Color Zones:**
- 🟢 **0-25%:** Green (Low impact)
- 🔵 **25-50%:** Blue (Moderate impact)
- 🟠 **50-75%:** Orange (High impact)
- 🔴 **75-100%:** Red (Critical impact)

**Features:**
- ✅ Normalized 0-100 scale (relative to max)
- ✅ Color-coded zones
- ✅ Animated needle
- ✅ Country-specific colors in value
...
function updateStoryline() {
    ...
    const countryInsights = {
        'US': [
            'Largest per-capita CO₂ emissions among developed nations',
...
**Features:**
- ✅ Zig-zag pattern alternates cards left/right
- ✅ Numbered timeline nodes connect sections
- ✅ Contextual insights on opposite side of cards
- ✅ Smooth hover animations
- ✅ Country-specific insights database
- ✅ Gradient background with glassmorphism
- ✅ Fully responsive (stacks on mobile)

**Design Pattern:**
```
Card (Left) ← → Timeline Node ← → Insights (Right)
Insights (Left) ← → Timeline Node ← → Card (Right)
```
...
**Features:**
- ✅ Concentric rings for different trade categories
- ✅ Country arcs sized by trade volume
- ✅ Color-coded by country
- ✅ Interactive hover tooltips
- ✅ SVG and PNG export with timestamp
- ✅ Syncs with global country and factor selection
...
**Features:**
- ✅ Chord diagram for directional flows (A → B)
- ✅ Ribbon thickness represents trade volume
- ✅ Color gradient represents environmental impact intensity
- ✅ Animated flow particles along paths
- ✅ Hover highlights specific trade routes
- ✅ Country labels around perimeter
- ✅ Color legend showing impact scale
- ✅ Export PNG and SVG with timestamp
- ✅ Syncs with global factor selection
...
<div class="factor-selector-sticky" id="factor-selector">
    <h3>🌍 Environmental Factor Group</h3>
...
<div class="selected-countries-section">
    ...
    <div class="carousel-nav" id="carousel-nav" style="display: none;">
        <button class="nav-btn" onclick="scrollCarousel(-1)" aria-label="Previous">◀</button>
        <button class="nav-btn" onclick="scrollCarousel(1)" aria-label="Next">▶</button>
    </div>
</div>
```
    ...
    return new Promise((resolve, reject) => {
        Papa.parse(url, {
            ...
            complete: (results) => {
                ...
                debugLog(`✓ Loaded ${url.split('/').pop()}: ${results.data.length} rows`);
                resolve(results.data);
            },
            error: (error) => {
                console.error(`Failed to load ${url}:`, error);
                loadingErrors.push({ url, error: error.message });
                debugLog(`✗ Failed to load ${url}: ${error.message}`, true);
...
async function loadAllData() {
    showStatus('🔄 Loading reference data...');
    ...
    if (industryData) {
        ...
        debugLog(`✓ Indexed ${Object.keys(industryLookup).length} industries`);
    }
    ...
    if (factorData) {
        ...
        debugLog(`✓ Indexed ${Object.keys(factorLookup).length} factors`);
    }
    
    // Step 2: Load country trade flow data
    showStatus('🌍 Loading country trade data...');
    ...
    showStatus(`✅ Data load complete${errorMsg}`, loadingErrors.length > 0);
...
async function loadCountryFlow(code, flow) {
    ...
    try {
        ...
        debugLog(`✓ Loaded ${code}/${flow}: ${trade ? trade.length : 0} trades, ${tradeFactor ? tradeFactor.length : 0} factors`);
    } catch (error) {
        debugLog(`✗ Failed ${code}/${flow}: ${error.message}`, true);
    }
}
...
- ✅ Return null instead of rejecting promises
...
**Problem:** Requesting data for non-existent countries causes 404 errors → retry loops → CPU overload → browser crash
...
function loadCountryDataSafe(countryCode) {
    ...
    if (!isValidCountry(countryCode)) {
        console.warn(`❌ Country ${countryCode} not in dataset, skipping`);
...
**Critical Rules:**
- ❌ NEVER request countries not in `AVAILABLE_COUNTRIES`
- ❌ NEVER retry failed requests automatically
- ✅ ALWAYS return null on failure, don't throw
- ✅ Log warnings, don't show error alerts
- ✅ Continue with remaining data loads
...
function disposeAllCharts() {
    // ... existing disposals ...
    disposeChart('mynewchart'); // ← Add here
}
```
...
const FACTOR_GROUP_MAPPING = {
    ...
    'biodiversity': ['Biodiversity', 'Species', 'Habitat', 'Ecosystem'] // ← New
};
...
**Requirements:**
1. ✅ Data must exist in dataset at `https://github.com/ModelEarth/trade-data/tree/main/year/2019/[COUNTRY_CODE]`
2. ✅ Must have all 6 required files (domestic, exports, imports × trade + trade_factor)
...
- **Style samples:** https://model.earth/cms/themes/
...
- ✅ Always validate country codes against `AVAILABLE_COUNTRIES`
- ✅ Handle errors gracefully without crashing
- ✅ Position legends outside charts to prevent overlap
- ✅ Use consistent color schemes across visualizations
- ✅ Sync chart updates with global state changes
- ✅ Provide export options (PNG/SVG) for major visualizations
- ✅ Implement responsive design with proper resize handling
- ✅ Add loading indicators for async operations
- ✅ Test with different numbers of selected countries (0, 1, 2, 3+)
- ✅ Dispose charts properly to avoid memory leaks
- ✅ Use debouncing for resize/scroll event handlers
- ✅ Log errors to debug panel, not alert dialogs
- ✅ Include tooltips with country flags and formatted numbers
- ✅ Keep chart libraries in CDN (no local dependencies)
- ✅ Document any new patterns in this CLAUDE.md file
...
- ✅ 8 reusable chart components with production code
- ✅ Complete UI/UX patterns library
- ✅ Data processing functions
